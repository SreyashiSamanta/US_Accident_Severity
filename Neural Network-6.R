...
sub_data$Severity <- factor(sub_data$Severity, levels = c(2, 3, 4))
...
#formula <- as.formula("Severity ~ .")
formula <- as.formula("Severity ~ Wind_Speed + Temperature + Visibility + Pressure + Month + Year  + Traffic_Signal + Bump + Crossing")


# Train the neural network model
model <- neuralnet(formula, data = train_data, hidden = c(5, 5), linear.output = FALSE)

# Make predictions on the test set
pred_prob <- compute(model, test_data[, -1])$net.result

pred <- rep(0, nrow(pred_prob))
pred[pred_prob[,2] > pred_prob[,1]] <- 2
pred[pred_prob[,3] > pred_prob[,2]] <- 3
pred[pred_prob[,4] > pred_prob[,3]] <- 4

# Create a confusion matrix
conf_matrix <- table(Actual = test_data$Severity, Predicted = pred, levels = c("2", "3", "4"))
print(conf_matrix)
