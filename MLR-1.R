...
sub_data <- subset(accident.df, select = c("Wind_Speed", "Visibility", "Temperature", "Pressure", "Weather_Condition", "Severity"))

#sub_data <- subset(accident.df, select = c("Severity", "Wind_Speed", "Temperature", "Visibility", "Pressure", "Month", "Year",  "Side", "Sunrise_Sunset", "Weather_Condition", "Traffic_Signal", "Bump", "Crossing"))
...
#Fit the model using multinomial logistic regression
model <- multinom(Severity ~ ., data = train_data)
